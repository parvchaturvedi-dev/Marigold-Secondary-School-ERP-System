...
import { MessageSquare, Reply, Send, UserRound, Users } from 'lucide-react';
import {
...
const Communication = ({ session }) => {
  ...
  const [messages, setMessages] = useState(() => getTeacherMessages(session));
  const [draft, setDraft] = useState('');
  const [recipient, setRecipient] = useState(sections[0]?.label || 'Academic Coordinator');
  ...
  const handleSubmit = (event) => {
    ...
    setDraft('');
    alert(`Message queued for ${recipient}.`);
  };
    ...
    <div className="space-y-6 pb-8 select-none font-sans text-[#1A1A1A]">
      <section className="bg-white border border-[#C8C8C8] rounded-3xl p-6">
        ...
        <p className="text-xs font-bold text-[#555555] mt-1">
          Faculty inbox and class communication for {profile.displayName}.
        </p>
      </section>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-5 gap-6">
          ...
          <div className="space-y-3">
            {messages.map((message) => (
              <article key={message.id} className="bg-[#F8F8F8] border border-[#EAEAEA] rounded-3xl p-4 space-y-2">
                ...
                <span className="inline-flex text-[9px] font-black px-2 py-1 rounded-md bg-white border border-[#EAEAEA] text-[#555555]">
                  {message.tag}
                </span>
              </article>
            ))}
        ...
        >
          ...
          <div className="bg-[#F8F8F8] border border-[#EAEAEA] rounded-2xl p-3 text-xs font-bold">
            ...
            <p className="text-[10px] text-[#555555] mt-1">{profile.employeeId} | {profile.department}</p>
          </div>
          ...
          <div className="space-y-1">
            ...
            >
              ...
              {sections.map((section) => (
                <option key={section.id}>{section.label} Parents</option>
              ))}
            </select>
          </div>
      ...
      <section className="bg-white border border-[#C8C8C8] rounded-3xl p-5">
        <h3 className="text-sm font-black flex items-center gap-2 mb-4">
          <Users className="w-4 h-4" /> Class Broadcast Groups
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
          {sections.map((section) => (
            <div key={section.id} className="bg-[#F8F8F8] border border-[#EAEAEA] rounded-2xl p-4">
              <p className="text-xs font-black">{section.label} Parents</p>
              <p className="text-[10px] font-bold text-[#555555] mt-1">
                {section.students} guardians | {section.subjects.join(', ')}
              </p>
            </div>
          ))}
        </div>
