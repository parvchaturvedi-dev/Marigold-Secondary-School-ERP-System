...
import { clerkModules, clerkBottomTabs } from "../data/clerkModules";
...
export default function ClerkDashboardScreen() {
  ...
  if (activeTab === "attendance") {
    return <AttendanceScreen role="clerk" onBack={() => setActiveTab("home")} />;
  }
        ...
        >
          ...
          {activeTab === "profile" && (
            <SimpleTabPage
              ...
              text={`${summary?.profile?.designation || "Office Administration"} account signed in as ${user?.name || user?.displayName || user?.username || "clerk"}.`}
              logout={logout}
            />
          )}
        </View>
...
function HomeContent({ user, summary, loading, error, onModulePress }) {
  ...
  const stats = summary?.stats?.length
    ? summary.stats
    : [
        { title: "Documents", value: loading ? "..." : "0", subtitle: "Vault", color: "#22C55E" },
        { title: "Students", value: loading ? "..." : "0", subtitle: "Enrolled", color: "#F97316" },
  ...
  return (
    <View style={{ marginTop: 36 }}>
      ...
      >
        <View>
          <Text style={{ fontSize: 18, color: palette.ink }}>{getGreeting()}</Text>
          <Text
            style={{
              fontSize: 26,
              fontWeight: "900",
          ...
          >
            {user?.name || "Clerk Portal"}
          </Text>
          <Text style={{ marginTop: 8, color: palette.inkLabel, fontSize: 16 }}>
            {profile.designation || "Office Administration"} | {profile.classLabel || "Office desk"}
          </Text>
        </View>
      ...
      >
        ...
        >
          <Ionicons name="folder-outline" size={30} color="#fff" />
        </LinearGradient>

        <View style={{ flex: 1 }}>
          <Text style={{ color: palette.ink, fontSize: 15, fontWeight: "800" }}>
            {action.label || "Office Records"}
          </Text>
          <Text
            style={{
              color: palette.ink,
        ...
        <TouchableOpacity
          onPress={() => onModulePress({ title: "Documents Management" })}
          style={{
